...
static long basler_camera_priv_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	{
		struct basler_capture_properties __user *uptr = (struct basler_capture_properties __user *)arg;
		struct basler_capture_properties csi_prop;

		ret = basler_retrieve_capture_properties(sensor, &csi_prop);
		if (ret < 0)
			return ret;

		ret = copy_to_user(uptr, &csi_prop, sizeof(struct basler_capture_properties));
		if (ret)
			return -EFAULT;
...
static int
basler_camera_g_ctrl_capture_properties(struct device *dev,
					struct basler_camera_dev *sensor,
					const struct v4l2_ctrl *ctrl)
{
	if (ctrl->elem_size == sizeof(struct basler_capture_properties)) {
		...
		return basler_retrieve_capture_properties(sensor, l_bcp);
	}
	return -ENOMEM;
}
...
static int basler_camera_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_BASLER_CAPTURE_PROPERTIES:
		return basler_camera_g_ctrl_capture_properties(dev, sensor, ctrl);
	default:
		break;
	}
